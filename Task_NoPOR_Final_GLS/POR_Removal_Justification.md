...
## Appendix: Recommended External Reset Circuit

```
┌────────────────────────────────────────────────┐
│   EXTERNAL RESET CIRCUIT (PCB Level)          │
└────────────────────────────────────────────────┘

Reset Button (Active Low / Open-Drain Source)
        |
        +──[R1: 10-100kΩ]─┬─────────────────→ SCL-180 Reset Pad
                          |
                       [C1: 0.1µF]
                          |
                         GND (VSS)
...
Component Values (Typical):
    R1 = 10kΩ (1% tolerance, 0.25W)
    C1 = 0.1µF (5-10% tolerance, 10V ceramic MLCC)
    
Time Constant: τ = 10kΩ × 0.1µF = 1ms
Settling Time: 10τ = 10ms

┌────────────────────────────────────────────────┐
│   INTERNAL RESET SYNCHRONIZATION (ASIC)       │
└────────────────────────────────────────────────┘

Reset_Pad_Input
        |
        +──→ [Sync FF1] ──→ [Sync FF2] ──→ [Sync FF3]
             (metastable    (synchronized   (synchronized)
              window)        to Clk)
        |
        +──→ Reset_Synchronized
             (distributed to all FFs in clock domain)

Repeat synchronizer for each independent clock domain.
```

**Timing Parameters**:
- Pad propagation delay: 500ps - 1ns (from library)
- Synchronizer settling: 2-3 clock cycles (20-100ns at high frequency)
- Reset assertion hold time: ≥50ns
